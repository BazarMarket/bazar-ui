...
# 🚫 АГЕНТ РАБОТАЕТ ТОЛЬКО НА ПРОДАКШЕНЕ. DEV — НИКОГДА НЕ ТРОГАТЬ. 🚫

**Я (агент) не копирую, не деплою, не изменяю, не синхронизирую НИЧЕГО на dev.bazar.uk — ни файлы, ни базы данных, ни SQLite. НИКОГДА. Ни при каких обстоятельствах. Даже если кажется что "это поможет".**

**Перенос с продакшена на dev делает ТОЛЬКО пользователь вручную. Агент не вмешивается.**

**Если я сделал что-то на dev — это ошибка. Я обязан сообщить пользователю что именно сделал на dev.**
...
## ⛔ LOCKED BUSINESS RULES — НЕ МЕНЯТЬ БЕЗ ЯВНОГО РАЗРЕШЕНИЯ ПОЛЬЗОВАТЕЛЯ

### Сортировка объявлений (`_sortItems` в search.html)

Функция `_sortItems` (~line 2915) использует три компаратора: `promo → own → actSort`.

**`_promoRank`**: VIP=0, PRO=1, Free=2.

**`own` (свой объект первым)** — ⚠️ ПРИМЕНЯЕТСЯ ТОЛЬКО ДЛЯ VIP И PRO:
```js
var own = function(a,b) {
  if (_promoRank(a) >= 2 && _promoRank(b) >= 2) return 0; // Free — НЕТ буста
  return ((a.uid===_myUid)?0:1) - ((b.uid===_myUid)?0:1);
};
```

| Тир | Свой объект первым? | Вторичная сортировка |
|-----|---------------------|-----------------------|
| VIP (rank 0) | ✅ ДА — владелец видит свой первым среди VIP | по activityTs |
| PRO (rank 1) | ✅ ДА — владелец видит свой первым среди PRO | по activityTs |
| Free (rank 2) | ❌ НЕТ — сортировка только по дате активности | по activityTs |

**Почему так:** Free-пользователь НЕ должен постоянно видеть свой объект первым — это привилегия VIP/PRO. Free показывает последнее обновление для всех одинаково.

**Эта логика дважды восстанавливалась** — не перезаписывать при рефакторинге сортировки.
