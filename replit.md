...
- **`card.html` блок продавца (`.card-name`):** Иконки мессенджеров перенесены из правой колонки (рядом с именем) в левую (под аватарку). 2-3 мессенджера → под аватаркой, аватарка вверх (класс `card-name__left--top`). 1 мессенджер → иконки не показываем, добавляем строку "Contacts: WhatsApp" в таблицу specs. 0 мессенджеров → иконки скрыты. `.card-name { min-height: 110px }`, `.card-name__left--top { margin-top: -8px }`, `.card-name .card-name__text { margin-top: auto !important }` (прижимает текст+кнопку вниз, выравнивая по иконкам), иконки: 28×28px, border-radius: 8px, font-size: 16px, gap: 4px — НЕ МЕНЯТЬ без явной просьбы (пиксельно выверено пользователем).
