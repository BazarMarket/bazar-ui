...
- **`post-ad.html` — `window.BAZAR_API` в `<head>`:** В самом начале `<head>` есть inline `<script>`, который определяет `window.BAZAR_API` и сразу вызывает `/is-admin` для показа admin-only категорий (Motors, Phones и т.д.). Этот inline-блок ОБЯЗАТЕЛЕН — без него `window.BAZAR_API` будет `undefined` в момент вызова (т.к. `script.js` загружается позже, в конце body). НЕ УДАЛЯТЬ и не переносить этот блок. На продакшне `/is-admin` требует активной сессии на `admin.bazar.uk`; в dev (`server.py`) всегда возвращает `{"admin":true}`.
