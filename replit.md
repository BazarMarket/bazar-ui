...
- **`card.html` блок продавца (`.card-name`):** Иконки мессенджеров перенесены из правой колонки (рядом с именем) в левую (под аватарку). 2-3 мессенджера → под аватаркой, аватарка вверх (класс `card-name__left--top`). 1 мессенджер → иконки не показываем, добавляем строку "Contacts: WhatsApp" в таблицу specs. `.card-name { min-height: 110px }` и `.card-name .card-name__text { margin-top: 4px !important }` — НЕ МЕНЯТЬ без явной просьбы (пиксельно выверено пользователем).
